...
trait ClassBase
{
	 ...
	 * @var string
	 */
	protected $_errorMode = 'exce';

	/**
	 * システム用例外クラス
	 * @var string
	 */
	protected $_sys_exception = '\feke\error\FekeError';

	/**
	 * ユーザー用例外クラス
	 * @var string
	 */
	protected $_user_exception = '\Error';
	...
	public function setConfig ($config)
	{
		...
		//その他ではオブジェクトへ変換して保存
		if (is_string($config)) {
			$this->_Config | \Feke::loadConfig($config);
		} else {
			$this->_Config = object_merge($this->_Config, to_object($config));
		}
	}
	 ...
	 * @param string $flag
	 */
	public function errorMode ($flag)
	{
		if ($flag == 'bool') {
			$this->_errorMode = 'bool';
		} else {
			$this->_errorMode = 'exce';
		}
	}
	 ...
	 * @param string $msg
	 */
	protected function throwError ($error, $system_fg = null)
	{
		if (is_string($error)) {
		...
		if ($this->_errorMode === 'bool') {
			...
			if($system_fg === true) {
				throw new $this->_sys_exception ($error);
			}
			else {
				throw new $this->_user_exception ($error);
			}
		}
	}
	 ...
	 * @return boolen
	 */
	public function usePlugin ($class_name = null)
	{
		if ($class_name) {
			$trait_list = class_uses($class_name);
			if (false !== ($class_list = class_parents($class_name))) {
				foreach ($class_list as $oya_class_name) {
					$trait_list += class_uses($oya_class_name);
				}
			}
		} else {
